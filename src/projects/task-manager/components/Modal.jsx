...
function Modal({ children, isOpen, onClose, title, showCloseButton = true, className = "" }) {
  ...
  return ReactDOM.createPortal(
    <div
      className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 transition-opacity duration-300 ease-in-out animate-fadeIn"
      onClick={onClose} // Close modal on backdrop click
      role="dialog"
      aria-modal="true"
    >
      <div
        className={`bg-slate-800 p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-md transform transition-all duration-300 ease-out animate-scaleUp ${className}`}
